...
* `frontend/src/scene/invalidation.ts` mirrors these cascades in 16 exported
  ...
  helpers) — a hand-maintained copy of the same graph, audited by AC-01I
  against the registry closure under both ramp sources: every production
  mutation site's frontend closure equals its backend row (no live drift);
  the one latent gap (`afterUpstreamRegen` omitted `tunnelMesh` while every
  backend upstream root deletes `tunnel_mesh.json`; only reachable through
  `afterRampChange`, which added it) is closed and the ramp-change helper is
  that same closure, so the module holds ONE mapping. It is declared a
  UX / cache-freshness mirror, never a correctness authority. The stale
  `developmentMeshScope.ts` comment naming the removed
  `DesignService._delete_levels_artifact` is corrected.
...
| F08 | `ci.yml` and `verify-full.yml` both run the whole gate set | **resolved by AC-01H**: `ci.yml` retired after same-revision equivalence on `c9f5913a0636` (old `pytest -q` 1241 tests / 0 failed vs new unfiltered `pytest-full` executed 1241 == collected, coverage proof satisfied; frontend five gates + vitest 40 files / 263 tests identical; triggers `pull_request` + `push: main` on both). `verify-full.yml` is the ONE release authority: two component jobs (`--backend-only`, `--frontend-only`, one runner implementation) publish real evidence artifacts (`include-hidden-files: true`, `if-no-files-found: error` — the pre-AC-01H uploads published 0 files) and `Release Authority` (needs both, `if: always()`) aggregates them with `verify.py authority` into one fail-closed verdict (`verification-release-authority`). Measured fail-closed on a real run: a cancelled backend job left a summary-less package and the verdict was withheld with `COMPONENT_SUMMARY_MISSING`, job red, artifact still published. Per PR revision: unfiltered backend pytest 2 → 1, frontend full gates 2 → 1 (42.0 → 40.5 runner-minutes). Residuals: a PR run certifies the synthetic merge commit (`PULL_REQUEST_MERGE_SIMULATION`) — exact-HEAD authority stays the local `verify.py full`; branch protection references no check (a future required check should be `Release Authority`); the aggregate emits `PYTEST_FULL_FILTERED` when NO component carries an unfiltered pytest, missing included — a `PYTEST_FULL_NOT_RUN` label would read better (pre-existing AC-01A wording, fail-closed either way) |
| F09 | typed-API principle vs `dict[str, Any]` layout payloads | **partially resolved by AC-01I**: the closed-shape boundaries are typed — `RampSourceSummary` (`services/effective_ramp.py`, `response_model` of `GET/PUT …/design/ramp-source`) and `LayoutActivateResponse` (`api/design.py`, `POST …/layout-v2/activate`) — with the serialized JSON pinned byte-equal (`tests/test_reference_contracts.py` B4, `tests/test_artifact_read_api.py` B5 / activate). INTENTIONALLY RETAINED as `dict[str, Any]`: every route that serves a persisted document raw (`layout_v2.json`, `layout_v2_selected.json`, `level_accesses.json`, the effective ramp, `decline*.json`, `tunnel_mesh.json`, `development_mesh.json`, the scene, job `result`) — those documents are written by earlier backend versions and carry open diagnostic sub-dicts, so an `ApiModel` (`extra="forbid"`, `allow_inf_nan=False`, declaration-order keys) would change their wire shape or reject old artifacts; and the async POST routes whose `?sync=true` variant answers a second shape. Residual: typing those needs a persisted-document schema decision (`extra="allow"` + write-time revalidation), not a DTO swap |
