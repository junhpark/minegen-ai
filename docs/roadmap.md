...
| 23A | MineExchange Core v1 — **DELIVERED** (rule 190, `docs/mine-exchange.md`) | versioned (`1.0.0`), read-only, deterministic interoperability bundle projected from authoritative state: terrain node grid (CSV / ESRI ASC / open TIN), orebody authority JSON + derived closed mesh (TABULAR / ELLIPSOID / WARPED_VEIN, QA'd), fault planes, individual closed excavation solids through the production sweep helpers (never unioned; `mine_multibody.stl` is a concatenation), verbatim render GLBs, centerlines CSV / DXF, network and capability DTOs; world-only exports with manifest omissions; `POST …/export/mine-exchange` (sync, no persisted artifact); Scenario-panel button. Stopes / mining-method contract deferred to 21A.2 |
| 21A | MiningMethodPlan + Longhole migration — **NEXT** | prove the new MiningMethodPlan abstraction with already-validated geometry |
| 21A.2 | MineExchange mining-method extension — PLANNED | stope / production-development / drawpoint / pillar / backfill entities as a 1.x extension of the MineExchange contract, after 21A fixes their ownership |
| 21B | Cut & Fill — PLANNED | lift / cut / backfill 4D mining method |
| 21C | Room & Pillar — PLANNED | limestone; pillar, room, bench, double bench |
| 22 | Analysis / Economics / Compliance — PLANNED | production, development, cost, revenue, cashflow + rule compliance + layout comparison |
